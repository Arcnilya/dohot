...
relays in Sweden places them in close proximity to the client. Regarding the
availability of Tor relays around the world, see Section~\ref{sec:limit}.
...
are expected to return an non-existing-domain return code (NXDOMAIN). The
query name consists of \texttt{prot-method-circuit-i-XXXXXXXX.net}. The first
part is the protocol in use (i.e, dohot, dotor, odoh). The second part is the
method for building circuits (i.e., torrc or carml). The third part represents
the built circuit using the country codes for each relay. A three relay circuit
is represented as: any2any2any, while a two relay circuit contstrained to
Sweden is represented as: se2se. In addition to these parameters we also
include an increasing number representing the measurement index (0-999) as well
as a random alphanumeric string to ensure that the query is unique to avoid
indluence of cache hits. This approach with unique NXDOMAIN queries ensures
that the observed latency mainly reflects the path between the client and the
resolver. 

For each configuration we repeat the measurement with 1,000 unique queries. In
each iteration, we start our measurement container with the specified
configuration and wait until a circuit is established and an arbitrary query
receives a successful response. This ensure that the Tor circuit is fully
established and that we have a fully functional \ac{dns} resolution over Tor.
We then issue a query for an A record using a unique, non-existent domain and
measure the time, in milliseconds, for the reply to return. Finally, we stop
the container before the next iteration begins. The code for the containers and
measurement script can be found on GitHub\footnote{Anonymized for submission}.
